...
ALLOWED_MIMETYPES_MANAGER_ADMIN = [
    ...
    # "application/msword", # Старый DOC - не поддерживаем
]
...
async def download_file(
    file: models.File = Depends(get_file_for_user), db: AsyncSession = Depends(get_db)
):
...
async def list_files(
    ...
    if not files:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Не найдено ни одного файла!"
        )
    return files
