...
def delete_file_from_s3(obj_name: str):
    ...
    s3_client.delete_object(Bucket=settings.s3_bucket_name, Key=obj_name)


def download_file_from_s3(obj_name: str) -> BytesIO:

    try:
        file_stream = BytesIO()

        s3_client.download_fileobj(settings.s3_bucket_name, obj_name, file_stream)

        file_stream.seek(0)

        return file_stream
    except Exception as e:
        print(f"Ошибка при скачивании файла {obj_name}: {e}")
        return None
